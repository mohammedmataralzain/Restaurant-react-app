function App() {
  ...
  const user = {
    subjects2: 5,
    subjects: [
      { subject: "math", mark: 45 },
      { subject: "data strutter", mark: 90 },
    ],
  };

  const arr = user.subjects.filter((item) => item.mark > 50);

  console.log(arr[0]?.mark);

  return (
      //spool
    <p>
      {user.subjects.filter((item) => item.mark > 50)[0].mark}{" "}
      <svg
        width="40"
        height="10"
        viewBox="0 0 40 10"
